...
public class PlantController {
	...
	@PostMapping("/updateMinMaxQuantitie")
	public ResponseEntity<String> updateMinMaxQuantities(@RequestBody  ReorderRules reorderRules) {
		...
		try {
			...
			if(reorderRulesTemplate!=null) {
				...
				return new ResponseEntity<String>("PartId not found. Please provide valid partId to update min and max quantities", HttpStatus.NOT_FOUND);
		}
		catch(NoSuchElementException e) {
			return new ResponseEntity<String>("PartId not found. Please provide valid partId to update min and max quantities", HttpStatus.NOT_FOUND);
		}
